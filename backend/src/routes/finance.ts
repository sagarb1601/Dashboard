...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const ending_year = new Date(
      extension_end_date ? extension_end_date : end_date
    );

    const duration_years = ending_year.getFullYear() - start.getFullYear() + 1;
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      for (const entry of expenditures) {
        ...
        if (fieldCheck.rows.length === 0) {
          throw new Error(
            `Budget field ${entry.field_id} is not mapped to project ${entry.project_id}`
          );
        }
      }
...
router.get(
  ...
  async (req, res) => {
    ...
    `;
      ...
      res
        .status(500)
        .json({ error: "Failed to fetch budget fields with grants" });
    }
  }
);
